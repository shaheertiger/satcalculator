...
const ModuleInput: React.FC<ModuleInputProps> = ({ label, value, max, onChange, color }) => {
  return (
    <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-slate-100 transition-all hover:shadow-md">
      <div className="flex justify-between items-center mb-2 sm:mb-3">
        <label className="text-xs sm:text-sm font-semibold text-slate-600 uppercase tracking-wider">{label}</label>
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        <input
          ...
          className={`flex-grow h-2 rounded-lg appearance-none cursor-pointer accent-${color}-600 bg-slate-200 touch-manipulation`}
        />
        <div className="w-14 sm:w-12 text-center">
          <input
            type="number"
            min="0"
            max={max}
